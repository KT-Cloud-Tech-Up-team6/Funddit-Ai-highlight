...
`③ 타임라인`과 `④~⑥ 쇼츠`는 **별개 기능**입니다. 하나만 써도 됩니다.

| | 타임라인 | 하이라이트 쇼츠 |
| --- | --- | --- |
| 목적 | 다시보기 탐색 | SNS 업로드용 영상 |
| 커버 범위 | 방송 전체 (빈틈 없음) | 고른 구간만 |
| 사람 개입 | 없음 — 자동 노출 | 판매자가 선택 |
| 엔드포인트 | `GET /timeline` | `GET /candidates` → `POST /select` → `GET /shorts` |
| 결과 파일 | `timeline.json` | `candidates.json`, `shorts.json` |
| 비용 (방송 1건) | 26원 | 21원 + 구간당 9원 |

- 타임라인은 `ready_for_selection` 시점에 **이미 완성**되어 있습니다. 별도 요청이 필요 없습니다.
- 쇼츠는 `POST /select`를 호출해야 생성이 시작됩니다.
- **타임라인 생성이 실패해도 쇼츠는 정상 진행됩니다.** 반대도 마찬가지입니다.
- 타임라인만 쓰는 화면이라면 `④~⑥`을 호출하지 않으면 됩니다.
